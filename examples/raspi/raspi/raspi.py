from apyture import Device, StateMachineMixin, LogParser, Uart, state
...
class RaspberryPi(StateMachineMixin, Device):
    ...
    def _do_idle(self):
        self.cpu.start()
        return 'WAIT_FOR_UBOOT'


    @state('WAIT_FOR_UBOOT')
    def _do_wait_for_uboot(self):
...
class ModelB(RaspberryPi):
    # Feels a bit awkward. Would a decorator be better?
